...
export async function check_selected_time(
  access_token,
  departureTime,
) {
  const intdHr = parseInt(departureTime.slice(0, 3), 10);
  const intdM = parseInt(departureTime.slice(3, 5), 10);
  const current_time = await GetCurrentTime(access_token);
  const cHr = parseInt(current_time.Hr);
  const get_result = await GetLowerBoundTime(access_token);
  const ub_time_hr = parseInt(get_result.three.slice(0, 3), 10);
  const ub_time_min = parseInt(get_result.three.slice(3, 5), 10);
  const b_time = parseInt(get_result.four);
  const start_hr = cHr + b_time;

  if (intdHr >= start_hr && intdHr <= ub_time_hr) {
    if (intdHr === ub_time_hr) {
      if (intdM <= ub_time_min) {
        return true;
      } else {
        return false;
      }
...
export async function check_selected_date(
  access_token,
  departureDate,
  arrivalDate
) {}

export async function check_blacklist(access_token) {}
